...
export async function finnhubPriceTarget(symbol, key){
  ...
  try{
    ...
    if(data && (data.targetHigh!=null || data.targetLow!=null || data.targetMean!=null || data.targetMedian!=null)){
...
export async function getAggregatedPriceTarget(symbol, finnhubKey, alphaKey, current){
  const errors = [];
  try{ return normalizeTargets(await finnhubPriceTarget(symbol, finnhubKey), current); }
  catch(e){ errors.push(e.message); }
  try{ return normalizeTargets(await yahooPriceTarget(symbol), current); }
  catch(e){ errors.push(e.message); }
  try{ return normalizeTargets(await alphaVantageTarget(symbol, alphaKey), current); }
  catch(e){ errors.push(e.message); }
