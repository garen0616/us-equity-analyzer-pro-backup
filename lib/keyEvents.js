...
function normalizeArticles(articles=[], baselineDate){
  const base = dayjs(baselineDate);
  return articles.map(a=>{
    let date = dayjs(a.published_at || a.date);
...
function normalizeSecFilings(filings=[], baselineDate){
  return filings.map(f=>({
    type: f.form === '8-K' ? 'regulatory' : 'filing',
    date: dayjs(f.filingDate).isValid() ? f.filingDate : baselineDate,
    title: `${f.form} filing`,
...
export async function buildKeyEvents({ ticker, cik, baselineDate, openKey, model }){
  ...
  try{
    ...
    const combined = [
      ...normalizeArticles(gdeltArticles, baselineDate),
      ...normalizeSecFilings(filings, baselineDate),
      ...alphaEvents
    ];
